package org.bealead.common.utils
...
object PreferenceDataStoreUtils {
    fun create(name: String): BasePreferenceDataStore {
...
fun PreferenceDataStore.intString(key: String, defValue: Int = 0): ReadWriteProperty<Any, Int> {
    return object : ReadWriteProperty<Any, Int> {
        override fun getValue(thisRef: Any, property: KProperty<*>): Int = getString(key, null)?.toIntOrNull() ?: defValue
        override fun setValue(thisRef: Any, property: KProperty<*>, value: Int) = putString(key, value.toString())
    }
}

fun PreferenceDataStore.longString(key: String, defValue: Long = 0): ReadWriteProperty<Any, Long> {
    return object : ReadWriteProperty<Any, Long> {
        override fun getValue(thisRef: Any, property: KProperty<*>): Long = getString(key, null)?.toLongOrNull() ?: defValue
        override fun setValue(thisRef: Any, property: KProperty<*>, value: Long) = putString(key, value.toString())
    }
}
